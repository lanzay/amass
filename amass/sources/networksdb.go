...
func NewNetworksDB(config *core.Config, bus *core.EventBus) *NetworksDB {
	...
	n.BaseService = *core.NewBaseService(n, "NetworksDB", config, bus)
	return n
}
...
func (n *NetworksDB) OnStart() error {
	...
	if n.API == nil || n.API.Key == "" {
		...
		n.SourceType = core.SCRAPE
		n.hasAPIKey = false
	}

	n.Bus().Subscribe(core.IPToASNTopic, n.SendASNRequest)
...
func (n *NetworksDB) executeASNQuery(asn int, addr string, netblocks []string) {
	...
	n.Bus().Publish(core.NewASNTopic, &core.ASNRequest{
		Address:     addr,
		ASN:         asn,
		Prefix:      prefix,
		CC:          cc,
...
func (n *NetworksDB) executeAPIASNQuery(asn int, addr string, netblocks []string) {
	...
	n.Bus().Publish(core.NewASNTopic, req)
}
...
func (n *NetworksDB) apiASNInfoQuery(asn int) *core.ASNRequest {
	n.SetActive()
